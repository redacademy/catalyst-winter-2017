 ...
 */
  ...
  */
 function catalyst_comment_count() {
  ...
  */
 function catalyst_entry_footer() {
 ...
 function catalyst_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'catalyst_categories' ) ) ) {
 		...
 		set_transient( 'catalyst_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so catalyst_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so catalyst_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in catalyst_categorized_blog.
  */
 function catalyst_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'catalyst_categories' );
 }
 add_action( 'edit_category', 'catalyst_category_transient_flusher' );
 add_action( 'save_post',     'catalyst_category_transient_flusher' );
  ...
  */
 function catalyst_comment_list( $comment, $args, $depth ) {
  ...
  */
 function catalyst_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
